...
A continuación, se presenta una tabla comparativa con los resultados estimados para el tiempo medio de adopción por color de gato, obtenidos a partir de tres modelos distintos:

1. Modelo **sin censura**: solo considera gatos adoptados.
2. Modelo **con censura**: incorpora gatos que aún no han sido adoptados.
3. Modelo **con censura + ajuste por edad**: considera censura y controla por edad al ingreso.

```{r}
# Preparar resumen de los tres modelos dinámicamente
results_tbl_nc <- tibble(
  Modelo = "Sin censura",
  Color = c("Negro", "Otro"),
  Media = c(mean(mu_negro_nc), mean(mu_otro_nc)),
...
results_tbl_edad <- tibble(
  Modelo = "Con censura y edad",
  Color = c("Negro", "Otro"),
  Media = c(mean(mu_negro), mean(mu_otro)),
...
results_comp <- bind_rows(results_tbl_nc, results_tbl_cens, results_tbl_edad)
print(results_comp)
```

Esta tabla permite observar cómo la inclusión de casos censurados y el control por edad afectan las estimaciones del tiempo medio de adopción. Se destaca que los gatos negros presentan sistemáticamente tiempos medios de adopción mayores que los de otros colores, y que omitir la censura tiende a subestimar dichos tiempos.
